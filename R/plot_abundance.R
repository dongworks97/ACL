  ...
  if(type=="N"){
    ...
    else
    {
      ...
      p <- ggplot2::ggplot(confidence_intervals_n, aes(x = Year, y = estimate)) +
        ...
        ggplot2::labs(y = "Relative abundance", x = "Year", title = "Total abundance Over Years with Confidence Intervals") +
        ggplot2::theme_minimal()+ theme(plot.title = element_text(hjust = 0.5))
    }
  }
  if (type=="NA"){
    ...
    if(!se)
    {
      ...
      p <- ggplot2::ggplot(na_long, aes(x = Year, y = number.age)) +
        ...
        ggplot2::labs(x = "Year", y = "Relative abundance", title = "Total abundance of different age groups per year") +
        ggplot2::theme_minimal()+ theme(plot.title = element_text(hjust = 0.5))
    ...
    else
    {
      ...
      p <- ggplot2::ggplot(confidence_intervals_na, aes(x = Year, y = estimate)) +
        ...
        ggplot2::geom_ribbon(aes(ymin = lower, ymax = upper),  fill = se_color,alpha = se_alpha) +
        ggplot2::facet_wrap(~AgeGroup, ncol = facet_ncol, scales = facet_scales) +
        ggplot2::labs(x = "Year", y = "Relative abundance", title = "Total abundance of different age groups per year with Confidence Intervals") +
        ggplot2::theme_minimal()+ theme(plot.title = element_text(hjust = 0.5))
  ...
  if(type=="NL"){
    ...
    if(!se)
    {
      ...
      p <- ggplot2::ggplot(NL_long, aes(x = Year, y = Count)) +
        ...
        ggplot2::labs(x = "Year", y = "Relative abundance", title = "Total abundance of different length bins Over Years") +
        ggplot2::theme_minimal()+ theme(plot.title = element_text(hjust = 0.5))
    }
    else{
      ...
      p <- ggplot2::ggplot(confidence_intervals_NL, aes(x = Year, y = estimate)) +
        ...
        ggplot2::geom_ribbon(aes(ymin = lower, ymax = upper),  fill = se_color,alpha = se_alpha) +
        ggplot2::facet_wrap(~LengthGroup, ncol = facet_ncol, scales = facet_scales) +
        ggplot2::labs(x = "Year", y = "Relative abundance", title = "Total abundance of different length bins Over Years with Confidence Intervals") +
        ggplot2::theme_minimal()+ theme(plot.title = element_text(hjust = 0.5))
