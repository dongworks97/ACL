  ...
  if(type=="B"){
    ...
    else
    {
      ...
      p <- ggplot2::ggplot(confidence_intervals_bio, aes(x = Year, y = estimate)) +
        ...
        ggplot2::labs(y = "Relative biomass", x = "Year", title = "Total Biomass Over Years with Confidence Intervals") +
        ggplot2::theme_minimal()+theme(plot.title = element_text(hjust = 0.5))
    }
  }
  if (type=="BL"){
    ...
    p <- ggplot2::ggplot(BL_long, aes(x = Year, y = Count)) +
      ...
      ggplot2::labs(x = "Year", y = "Relative biomass", title = "Total Biomass at length group Over Years") +
      ggplot2::theme_minimal()+theme(plot.title = element_text(hjust = 0.5))
